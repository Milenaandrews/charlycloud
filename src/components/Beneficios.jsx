    ...
    <section id='beneficios' className="flex flex-wrap  h-full w-full pt-[70px] justify-items-center justify-center   ">
      {beneficios.map((item) => (
        <motion.div key={item.title} whileHover={{ scale: 1.1 }}
          ...
          <div className='flex h-full w-full absolute rounded-md transition duration-500 transform group-hover:scale-110 group-hover:rotate-3'>
            <Image
              fill
              alt={item.title}
          ...
          <div className='flex flex-col justify-content text-center bottom-0 gap-1 bg-charly2 w-72 h-40 absolute rounded  transition-all duration-500 p-3 text-white bg-opacity-50 group-hover:h-full group-hover:bg-opacity-100'>
